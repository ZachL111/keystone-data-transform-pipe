...
`keystone-data-transform-pipe` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies transform behavior through negative fixtures, with human-readable error snapshots and synthetic fixtures only.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 5.
